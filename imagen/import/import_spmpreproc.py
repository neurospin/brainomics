...
#        if (study_eid==None):
#            try:
#                c.cursor().execute('INSERT Study S: S name \'Imagen\',
#                S data_filepath \'\', S description \'Imagen\'')
#            except:
#                c.rollback()
...
#                print 'req = ', req
            ...
            S label \'%(e)s\', S type \'NIFTI\', S format \'NIFTI COMPRESSED\',
            ...
            res = c.cursor().execute('''SET S concerns Y Where S is Scan,
            Y is Subject, S identifier \'%(a)s\', Y identifier \'IMAGEN_%(b)s\'
            ''' % {'a': identifier, 'b': psc})
#           print 'res = ', res
#            res = c.cursor().execute('''SET S uses_device D Where S is Scan,
#            D is Device, S identifier \'%(g)s_%(a)s\',
#            D identifier \'%(c)s_%(d)s_%(e)s_%(f)s\'
#            '''%{'a': psc, 'b': cw_scanner_text,'c': cw_scanner_text,
#            'd': cw_scanner_manufacturer, 'e': cw_scanner_model,
#            'f': psc_center[psc], 'g': scan})
            #print 'res = ', res
