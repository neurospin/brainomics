...
def get_subjects_fam(filename):
    ...
    with open(filename) as infile:
        for line in infile:
            subject = dict()
            words = line.split(" ")
            subject["identifier"] = unicode(words[1])
            gender_code = 4 # words[4]
...
def get_cng_center():
    center = {}
    center["identifier"] = u"PartnerChip"
    center["name"] = u"PartnerChip"
...
def select_platform_from_filepath(filepath):
    ...
    elif u"Affymetrix6.0" in filepath:
        return get_platform_from_name(platforms, PLATFORM_AFFYMETRIX_6_0)
    return None
...
def select_db_platform_from_filepath(filepath, db_platforms):
    if u"illumina_610quad" in filepath:
        ...
        return get_db_platform_from_name(db_platforms, PLATFORM_AFFYMETRIX_6_0)
    return None
...
def import_studies(genomic_measures):
    study = {"name": u"SENIOR", "data_filepath": u""}
...
if __name__ == "__main__":
    root_path = "/neurospin/senior/genetics/adn/result_dir/to_index"
    genomic_platform_measure_path = os.path.join(root_path)
