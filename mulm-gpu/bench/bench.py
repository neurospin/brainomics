...
def test(X, Y, Z, n_perm):
    ...
    res1_ = res1[i1[::-1]][:end]
    res2_ = res2[i2[::-1]][:end]
    np.testing.assert_almost_equal(res1_['data'], res2_['score'], decimal=4)
    s1 = s2 = 0
    ...
    for perm_id in range(0, n_perm):
        ...
        np.testing.assert_almost_equal(d1[~mask], d2[~mask], decimal=4)
        s1 = e1
        s2 = e2
...
if __name__ == '__main__':
    ...
    X = np.load(x_files)['arr_0']
    Y = np.load(y_files)['arr_0']
    Z = np.load(z_files)
