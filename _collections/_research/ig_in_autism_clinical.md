...
With the use of machine learning methods, various methodological approaches will be applied to characterize ASD into specified subtypes based on three modalities: cognitive assessments, genetic SNPs and neuroimaging features. This will be done using the Healthy Brain Network (HBN) Cohort and will entail three major steps, the first two of which will be conducted on each modality. First, an unsupervised approach will be done using multiple correspondence analyses to uncover hidden patterns and trends. Next, based on what was ‘learned’ from the original data in the previous step, these hidden patterns will help us decide which features to include in the supervised learning model in order to develop an algorithm which will accurately predict ASD severity from new input data. Lastly, these methodologies will be implemented to robustly determine a stratification of ASD subtypes using a multi-block approach.
